...
운영 중인 Cafe24 단일 노드의 접근 권한을 관리하는 인프라 담당자용 문서다.
...
팀원에게는 [DB 접속 가이드](../guides/db-access.md)를 전달한다.
...
팀원에게 [개인 키 준비와 접속 요청](../guides/db-access.md#1-개인-키-준비와-접속-요청)을 안내하고,
원하는 SSH 계정명, `.pub` 공개키 전체 한 줄, 필요한 DB 환경과 읽기·쓰기 범위를 받는다.
계정명은 `jimin`처럼 영문 소문자로 시작하는 2~31자로, 영문 소문자·숫자·밑줄만 사용한다.
개인키와 Passphrase는 받지 않는다.

### 2. 현재 DB 목적지 확인

개인 `admin` SSH로 서버에 접속해 승인된 환경의 현재 Service ClusterIP를 조회한다.
...
`10.43.x.x:5432` 형식의 정확한 목적지만 허용하며 와일드카드는 사용할 수 없다.
Service를 재생성하거나 클러스터를 이전하면 IP를 다시 확인한다.
서버 OS에서 `*.svc.cluster.local`을 해석한다고 가정하지 않는다.

### 3. 기존 계정 목록에 추가

운영 inventory의 `ssh_access_users` 목록에 다음 항목을 추가한다.
아래는 전체 inventory가 아니라 추가 항목의 예시다. 공개키·주소를 실제 값으로 바꾸고
승인하지 않은 환경은 `permit_open`에서 제외한다. Preview가 승인되면 앞에서 조회한
Preview Service IP의 `:5432` 목적지도 추가한다.
...
환경별 PostgreSQL 계정·비밀번호·DB 이름을 별도로 전달한다.
비밀번호는 승인된 비밀 전달 수단으로 제공하며 Git·채팅방에 남기지 않는다.
[DataGrip 설정](../guides/db-access.md#2-datagrip-설정)에 따라 실제 DB 연결을 검증한다.
...
3. [관리자 PC 적용 명령](#4-관리자-pc에서-적용)으로 대상·문법을 확인한 뒤
...
SSH 변경 뒤에는 인증 로그와 방화벽도 확인한다. 공인 SSH `22/tcp`·HTTPS `443/tcp`만 열고,
DB `5432/tcp`·Kubernetes API `6443/tcp`는 외부에 공개하지 않는다. kubeconfig는 PC로 복사하지 않는다.
그 밖의 작업은 [문서 목차](../README.md)에서 찾으며, 접속 장애를 이유로 DB/PVC나 K3s를 초기화하지 않는다.
