...
| [`bootstrap_aws_secrets.py`](bootstrap_aws_secrets.py) | worksheet 검증과 26개 Secrets Manager source 생성 | 조회 또는 AWS Secrets Manager | Secret 최초 구성 |
...
ExternalSecret 계약과 일치하는 26개 JSON source를 구성한다.
...
| [`test_bootstrap_aws_secrets.py`](tests/test_bootstrap_aws_secrets.py) | worksheet parser, 26개 source mapping, 환경 분리, stdin 전달과 오류 메시지 비노출 |
